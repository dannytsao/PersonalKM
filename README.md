...
**Status:** ✅ LLM-Wiki v2 Live (AI distillation + entity dedup + bidirectional wikilinks)
...
| 文件 | 用途 |
|------|------|
| `LLMWIKI-INTEGRATION-CHANGENOTE.md` | **[NEW 2026-06-13]** LLM Wiki (Karpathy pattern) 整合說明、檔案清單、變更紀錄 |
...
**LLM-Wiki v2 上線** — 知識攝入管線全面升級。

- 新增 `bot/llm_summarizer.py`：MiniMax AI 濃縮摘要 + fallback，wiki 頁面從平均 3,000+ chars 降至 300-500 chars。
- 新增 `bot/entity_dedup.py`：`EntityRegistry` 索引 323 個既有 entity 頁面，相同主題的新 capture 自動合併，不再重複建檔。
- 新增 `bot/wikilinks.py`：雙向 wikilink，新增頁面連結到 entity，相關 entity 頁面自動加入 backlink。
- 新增 `bot/ingestion_v2.py`：整合上述 4 個 Phase 的攝入管線，取代 legacy `ingestion.py`。
- 新增遷移腳本：`scripts/migrate_wiki_to_v2.py`（375 頁面已遷移，節省 536,571 chars）、`scripts/fix_broken_wikilinks.py`（清理 42,479 chars）。
- cron 排程調整：週日 09:00 UTC+8 → **週六 23:00 UTC**（週日 07:00 UTC+8）。
- `wiki/` 現有 376 頁面（215 entities、161 concepts），0 失效 wikilink，health check: healthy。
- 詳細計畫：`docs/llm-wiki-v2-plan.md`。
...
| **每週六 23:00 UTC** (週日 07:00 UTC+8) | raw/ → wiki/ (LLM-Wiki v2 自動組織 + AI 濃縮 + entity 去重 + 雙向 wikilink) |
| **每月** | 生成衰退檢測報告 |
| **持續** | Git 自動提交 |
...
- [x] Phase 2: AI Summarization (`bot/llm_summarizer.py` — 375 頁面已遷移，~80% body 壓縮)
- [x] Phase 3: Entity Deduplication (`bot/entity_dedup.py` — 323 entities 索引)
- [x] Phase 4: Bidirectional Wikilinks (`bot/wikilinks.py` — 0 失效連結)
- [x] Phase 5: Full Integration (`bot/ingestion_v2.py`)
- [x] Wiki vault health: 376 頁面，0 broken wikilinks
