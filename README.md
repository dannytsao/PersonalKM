...
**Last Updated:** 2026-08-31 — Stage 1 P10 complete (YouTube anchors, stop-words, graph cleansing). AIGC keyword locking deployed. Dual-vault routing stabilized. Nearby MVP requirements and SDD are documented for the next implementation phase.
...
**Capture → Ingest → Distill.** 把 LINE 群組裡的連結與長文，自動轉成對「第二大腦」有用的結構化 Obsidian 筆記，並隨著多份 capture 不斷深化同一個實體的知識。

### 三個目標層級（must-have）

| # | 目標 | 驗收條件 |
|---|---|---|
| **G1** | **Capture**：每則 LINE 訊息都會變成 raw 檔案並 push 上 GitHub | Render webhook 收到 → raw/<日期>-<log_id>-<slug>.md 出現、可見、可重玩 |
| **G2** | **Ingest (LLM)**：raw 透過 LLM 產生符合 schema 的 wiki 筆記 | 每篇 wiki 頁面有：(a) LLM 生成的 3-5 句英文摘要 (b) topic ∈ 5 個選項之 1 (c) tags 為 cross-topic 標籤 (d) ≥1 source 指回 raw (e) confidence 非 `low`（除非真的沒字幕/沒內容）。**`skip_llm=True` 視為 pipeline 失敗** |
| **G3** | **Knowledge Distillation**：重複主題的 capture 必須自然累積在同一個 canonical entity page | 同一 entity 的多份 capture → 同一個 `entities/<slug>.md`；後續 capture 透過 `_propagate_to_entity_pages` 把 excerpt append 到該 entity 的 `## Captures`；query engine 對該 entity 回得出多份 capture 的綜合答案 |

### Schema contract (v1)

```yaml
---
title: <Human-readable title from H1, NOT from filename>
canonical: true          # entity pages only
created: YYYY-MM-DD
updated: YYYY-MM-DD
type: entity | concept
topic: <one of five fixed topics>
tags: [<cross-topic tags only, max 5>]
sources:
  - [[raw/archive/<subfolder>/<file-stem>]]   # Obsidian-clickable
confidence: high | medium | low
---

## Summary
<LLM-distilled 3-5 sentence English summary>

## Key Facts
- <concrete, specific fact>

## Captures
### <capture-stem> (<timestamp>)
<excerpt + backlink to source>
```
...
| Phase | 對應目標 |
|---|---|
| Phase A (`ingest_wiki.py`) | G2 — 必須用 LLM，否則只算「補_ca後 raw → placeholder」 |
| Phase B (`post_link_ollama.py`) | G3 — ollama 生成 wikilinks/backlinks |
| Phase 6 (`canonical entity`) | G3 — dedup + propagation 完成才算 distillation |
| Sanity check | 防止 frontmatter drift；不能當作主要管道 |

> **任何 phase 的「完成」只認真的 capture 跑通時的輸出 — 不是 code 在、不是 docs 寫。當 LLM provider = none 或 distillation fallback 走到 "first paragraph"，目標視為未達成。**
...
python3 scripts/ingest_wiki.py --vault ~/~/Documents/PersonalKM/Personalkm-vault

### 手動觸發 Phase B

```bash
cd ~/Documents/GitHub/DannyTsao/PersonalKM
python3 scripts/post_link_ollama.py --vault ~/~/Documents/PersonalKM/Personalkm-vault
```
