...
2. 安裝步驟簡易，包含連接模型提供者（如 Open Router）及設定通訊方式（如 Telegram），並可自訂工作空間與外觀。
