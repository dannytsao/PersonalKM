...
async def analyze_on_capture(note_path: Path) -> None:
    """Analyze newly captured note for conflicts with older notes."""
    try:
        metadata = get_note_metadata(note_path)
        if not metadata or not (metadata["is_devops"] or metadata["is_ai"]):
            return
        
        # If new note, check if it makes older notes obsolete
        if metadata["days_old"] == 0:  # Just created
            logger.info(f"New {metadata.get('title', 'note')} might make older notes obsolete")
            # This would trigger relationship analysis
            
    except Exception as e:
        logger.error(f"Error analyzing new capture: {e}")
