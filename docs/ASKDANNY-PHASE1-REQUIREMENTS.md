...
若地區或類型不足以產生可靠結果，可以追問一個最關鍵的澄清問題；不在 Phase 1 建立複雜多輪旅遊規劃狀態。

### FR-04：回答格式

回覆使用 LINE 純文字，最多列出 3–5 個相關候選。簡單查詢應優先使用 deterministic renderer；LLM 只負責必要的簡短語句整理，不決定候選集合。
...
地址不再作為獨立輸出欄位；沒有 GPS 時不得自行從地址推算座標。來源名稱仍由訊息尾端統一標示。
...
1. 已授權親友查詢「嘉義阿里山有什麼住宿？」時，只回傳已發布資料，並保留資料缺漏說明。
