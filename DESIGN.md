...
`~/Documents/GitHub/DannyTsao/PersonalKM`（開發/文件）和 `~/~/Documents/PersonalKM/Personalkm-vault`（vault）是不同的 repo。後者為 private repo `github.com/dannytsao/Personalkm-vault`。Phase A/B script 讀寫 vault repo，Render 和 Mac Mini 之間通過 GitHub 同步。
...
4. 手動執行：`python3 scripts/ingest_wiki.py --vault ~/~/Documents/PersonalKM/Personalkm-vault`
...
~/Documents/PersonalKM/Personalkm-vault
